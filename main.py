...
# Центр часов и длина стрелок
clock_center = (410, 120)  # Примерный центр циферблата
second_hand_length = 35  # Уменьшаем длину секундной стрелки
minute_hand_length = 27  # Уменьшаем длину минутной стрелки
hour_hand_length = 23    # Уменьшаем длину часовой стрелки  
...
def draw_clock(screen, time_now):
    ...
    screen.blit(clock_image, (360, 70))  # Примерное расположение
    ...
    # Вычисляем координаты конца секундной стрелки
    ...
    # Вычисляем координаты конца минутной стрелки
...
class AttendanceApp:
    def __init__(self):
        ...
        self.fps = 60
        self.show_splash_screen = True  # Флаг для отслеживания текущего экрана

        # Ініціалізуємо екран заставки та екран таблиці
        ...
        # Передаем callback-функцию add_student в TableScreen
    ...
    def run(self):
        ...
        while running:
            if self.show_splash_screen:
                # Отображаем заставку и обрабатываем события на заставке
                self.handle_splash_screen_events()
                self.splash_screen.display()
                time_now = datetime.now()
                draw_clock(self.screen, time_now)
            else:
                # Отображаем таблицу и обрабатываем события в таблице
    ...
    def handle_splash_screen_events(self):
        """ Обрабатываем события на экране заставки (переключение студентов). """
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    self.show_splash_screen = False  # Переключаемся на таблицу
                elif event.key == pygame.K_RIGHT:
                    self.splash_screen.next_student()  # Переход к следующему студенту
                elif event.key == pygame.K_LEFT:
                    self.splash_screen.previous_student()  # Переход к предыдущему студенту
    ...
    def handle_table_screen_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                self.table_screen.handle_event(event)  # Передаем событие для обработки кнопки
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    for student in self.students:
                        student.calculate_attendance()
                        # Обновляем статус HonorStudent или RegularStudent
                    ...
                    self.show_splash_screen = True  # Возвращаемся на заставку
                else:
                    self.table_screen.handle_event(event)  # Передаем событие в таблицу



    def add_student(self):
        name = self.get_input("Enter student name: ")  # Запрашиваем имя
        gender = self.get_input("Enter student gender (male/female): ")  # Запрашиваем пол

        if gender in ['male', 'female']:
            self.table_screen.add_student(name, gender)  # Добавляем студента через TableScreen
        else:
